...
def process_experiment_results(
    experiment_dirs: List[Path], show_progress: bool = True
) -> pd.DataFrame:
    """
    ...
    iterator = (
        tqdm(experiment_dirs, desc="Loading experiments")
        if show_progress
        else experiment_dirs
    )
    ...
    if results_df["difficulty"].dtype == "object":
        try:
            ...
            difficulty_map = {
                d: i + 1
                for i, d in enumerate(sorted(results_df["difficulty"].unique()))
            }
...
def calculate_accuracy_statistics(results_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    accuracy_by_difficulty = (
        results_df.groupby("difficulty")["is_correct"]
        .agg(["mean", "count"])
        .reset_index()
    )

    # Calculate accuracy by difficulty and question type
    accuracy_by_difficulty_type = (
        results_df.groupby(["difficulty", "question_type"])["is_correct"]
        .agg(["mean", "count"])
        .reset_index()
    )
    ...
    question_type_accuracy = (
        results_df.groupby("question_type")["is_correct"]
        .agg(["mean", "count"])
        .reset_index()
    )
    ...
    best_difficulty = accuracy_by_difficulty.loc[
        accuracy_by_difficulty["mean"].idxmax()
    ]
    worst_difficulty = accuracy_by_difficulty.loc[
        accuracy_by_difficulty["mean"].idxmin()
    ]
    best_question_type = question_type_accuracy.loc[
        question_type_accuracy["mean"].idxmax()
    ]
    worst_question_type = question_type_accuracy.loc[
        question_type_accuracy["mean"].idxmin()
    ]
...
def plot_accuracy_by_difficulty(
    ...
    exp_difficulty_stats = (
        results_df.groupby(["experiment_name", "difficulty"])["is_correct"]
        .mean()
        .reset_index()
    )
    ...
    difficulty_stats = (
        exp_difficulty_stats.groupby("difficulty")["is_correct"]
        .agg(["mean", "std", "count"])
        .reset_index()
    )

    # Calculate standard error
    difficulty_stats["se"] = difficulty_stats["std"] / np.sqrt(
        difficulty_stats["count"]
    )
...
def plot_accuracy_by_question_type_and_difficulty(
    results_df: pd.DataFrame,
    ...
    exp_difficulty_type_stats = (
        results_df.groupby(["experiment_name", "difficulty", "question_type"])[
            "is_correct"
        ]
        .mean()
        .reset_index()
    ...
    difficulty_type_stats = (
        exp_difficulty_type_stats.groupby(["difficulty", "question_type"])["is_correct"]
        .agg(["mean", "std", "count"])
        .reset_index()
    )

    # Calculate standard error
    difficulty_type_stats["se"] = difficulty_type_stats["std"] / np.sqrt(
        difficulty_type_stats["count"]
    )
    ...
    ax = sns.barplot(
        ...
        y="mean",
        hue="question_type",
        data=difficulty_type_stats,
        palette="Set2",
        hue_order=question_type_order,
        errorbar=None,
    )
    ...
    # Add custom error bars
    for _i, row in difficulty_type_stats.iterrows():
        # Calculate the patch index based on difficulty and question type
        diff_idx = list(difficulty_type_stats["difficulty"].unique()).index(
            row["difficulty"]
        )
...
def plot_accuracy_by_form(
    ...
    exp_form_accuracy = (
        results_df.groupby(["experiment_name", "form", "permutation"])["is_correct"]
        .mean()
        .reset_index()
    )
    ...
    form_accuracy = (
        exp_form_accuracy.groupby("form")["is_correct"]
        .agg(["mean", "std", "count"])
        .reset_index()
    )
    ...
    form_accuracy = form_accuracy.sort_values(
        "form", key=lambda x: [form_order.index(i) for i in x]
    )
    ...
    # Add custom error bars
    for _i, row in form_accuracy.iterrows():
...
def plot_accuracy_by_form_and_question_type(
    results_df: pd.DataFrame,
    ...
    results_df = results_df.dropna(
        subset=["base_form", "permutation", "question_type", "is_correct"]
    )
    ...
    permutation_stats = results_df.groupby(
        ["base_form", "permutation", "question_type"], as_index=False
    )["is_correct"].agg(mean="mean", count="count")

    # Aggregate across permutations for each base_form × question_type
    form_type_stats = permutation_stats.groupby(
        ["base_form", "question_type"], as_index=False
    )["mean"].agg(avg_accuracy="mean", accuracy_std="std", num_permutations="count")
    ...
    form_type_stats["question_type"] = pd.Categorical(
        form_type_stats["question_type"], categories=question_type_order, ordered=True
    )
    ...
    form_type_stats["se"] = (
        form_type_stats["accuracy_std"]
        / np.sqrt(form_type_stats["num_permutations"].clip(lower=1))
    ).fillna(0.0)
    ...
    ax.set_title(
        "Model Accuracy by Form and Question Type with Permutation Variation",
        fontsize=16,
    )
...
class ExperimentAnalyzer:
    ...
    def load_experiments(self, show_progress: bool = True) -> pd.DataFrame:
        ...
        self.results_df = process_experiment_results(
            self.experiment_dirs, show_progress
        )
        logger.info(
            f"Loaded {len(self.results_df)} question results from {len(self.experiment_dirs)} experiments"
        )
    ...
    def plot_all(self, save: bool = True) -> Dict[str, Tuple]:
        ...
        # Plot accuracy by question type and difficulty
        plots["question_type_difficulty"] = (
            plot_accuracy_by_question_type_and_difficulty(
                self.results_df,
                save=save,
                filename="accuracy_by_question_type_and_difficulty",
            )
